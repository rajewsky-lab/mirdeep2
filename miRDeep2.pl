...
getopts("a:b:cdt:uvq:s:z:p:g:",\%options);
...
##################################################
    ...
    if($file_precursors !~ /none/){
	    ...
	    my $dopt="";
	    if($options{'d'}){$dopt="-d";}
            my $quant = "quantifier.pl -p $file_precursors -m $file_mature_ref_this_species  -r $file_reads $file_star $species -y $time -k $dopt";
            print STDERR $quant,"\n";
            `$quant`;
            ...
            print STDERR "Pre-quantitation is skipped caused of missing file with known miRNAs\n\n\n";   
        }

    }else{
        print STDERR "Pre-quantitation is skipped caused of missing file with known precursor miRNAs\n\n\n";   
    }
...
sub prepare_signature{
    ...
    if($file_mature_ref_this_species !~ /none/i){
        ...
        print STDERR "prepare_signature.pl $file_reads $dir_tmp/precursors.fa -o $dir_tmp/signature.arf 2> /dev/null\n\n";
        start();
...
sub fold_precursors{
    ...
    start();
    my $ret_fold_precursors=`RNAfold < $dir_tmp/precursors.fa -noPS > $dir_tmp/precursors.str`;
    end();
    return;
}
