...
sub read_stats{
    ...
    printf STDERR "%7s %10s %10s %10s %10s %10s\n","#desc: ","total" ,"mapped","unmapped","%mapped","%unmapped";
    printf STDERR "%7s %10d %10d %10d "         ,"total: ",$count,$count2,$count-$count2;
    printf STDERR "%10.3f %10.3f\n",100*$count2/$count,100*(1-($count2/$count));
    foreach(sort keys %k2){
        $k22{$_}||=0;
        printf STDERR "%7s %10d %10d %10d ", "$_: ",$k2{$_},$k22{$_},$k2{$_}-$k22{$_};
